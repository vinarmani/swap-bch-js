...
class Swp {
    ...
    async uploadSignal(msgType, // exchange = 1,  escrow = 2
        ...
        if (msgType == 1) {
            ...
            configEmptyMetaOpReturn.exactUtxoIndex = dataObj.exactUtxoIndex ? 1 : 0
    ...
    async uploadPayment(fundingUtxo,                    // object in form: { txid:'', satoshis:#, vout:# }
        ...
        let configEmptyMetaOpReturn = {
            msgClass: 2,
            msgType: subscriptEnc ? 2 : 1, // 1 = exchange, 2 = collaborate (P2SH escrow)
            fileSize: fileSize,
            chunkCount: chunkCount,
            signalSha256Hex: signalSha256Hex,
            subscriptEnc: subscriptEnc,
            chunkData: null
        };
    ...
    async downloadTx(swapUri, decryptWif=null, progressCallback=null) {
        ...
        if(decryptWif) {
            ...
            // Decode subscript
            if(bfpMsg.subscriptEnc) {
                let subStruct = utils.convertToEncryptStruct(bfpMsg.subscriptEnc)
    ...
    static buildMetadataOpReturn(config) {
        ...
        if (config.msgClass == 2) {
            ...
            if (config.msgType == 2) {
                ...
                let subscriptEnc = Buffer.from(config.subscriptEnc, 'hex');
    ...
    parsebfpDataOpReturn(hex) {
        ...
        if(script[3] == 'OP_2') {
            chunkDataIndex = 7
            bfpData.subscriptEnc = Buffer.from(script[6], 'hex')
        }
