...
class BfpNetwork {
    constructor(BITBOX, grpcUrl=null) {
        ...
        if(grpcUrl) {
            ...
            this.client = new bchrpc.GrpcClient() // grpcUrl defaults to "https://bchd.greyh.at:8335"
    }
    ...
    async getLastUtxoWithRetry(address, retries = 40) {
		...
		while(result == undefined){
			...
			count++;
			if(count > retries)
				throw new Error("Experienced a problem getting last UTXO");
			await sleep(250);
		}
		return result;
    }
    ...
    async getUtxosGrpc (address) {
        ...
        const outs = bchUtxos.toObject().outputsList.map(out => {
            ...
            return { 
                ...
                satoshis: out.value,
                height: out.blockHeight > 10 ** 9 ? null : out.blockHeight,
    ...
    async sendTx(hex, log=true) {
        ...
        if(log)
            console.log('Bitbox sendTx() res: ', res);
        return res;
    }
    ...
    async sendTxGrpc(hex, log=true) {
        ...
        if(log)
            console.log('BCHD sendTx() res: ', resObj.hash);
        return resObj.hash
    }
