...
class BfpNetwork {
    ...
    async getLastUtxoWithRetryGrpc(address, retries = 40) {
		let result;
        let count = 0;
		while(result == undefined){
            result = await this.getLastUtxoGrpc(address)
			count++;
			if(count > retries)
				throw new Error("BCHD experienced a problem retrieving last UTXO");
			await sleep(250);
		}
		return result;
    }
    ...
    async getLastUtxoGrpc(address) {
        let res = await this.getUtxosGrpc(address)
        return res[0];
    }
    ...
    async sendTxWithRetryGrpc(hex, retries = 40) {
        let res;
        let count = 0;
        while(res === undefined || res.length != 64) {
            res = await this.sendTxGrpc(hex);
            count++;
            if(count > retries)
                break;
            await sleep(250);
        ...
        if(res.length != 64)
            throw new Error("BCHD network error");
