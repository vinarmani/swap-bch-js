...
module.exports = class BfpBitdb {
    ...
    constructor(network) {
        ...
        this.jqQueries = {
            signal: [
                ...
                { "f": "[ .[] | { timestamp: (if .blk? then (.blk.t | strftime(\"%Y-%m-%d %H:%M\")) else null end), oracleBfp: \"bitcoinfiles:\\(.out[0].h4)\", contractTermsIndex: .out[0].h5, contractPartyIndex: .out[0].h6, compilerId: .out[0].s7, CompilerVersion: .out[0].s8, pubKey: .out[0].h9, exactUtxoTxId: .out[0].h10, exactUtxoIndex: .out[0].h11, appendedScriptPubKey: .out[0].h12, appendedSats: .out[0].h13, URI: \"swap:\\(.tx.h)\" } ]" } // Signal / Escrow
            ],
            payment: [
                { "f": "[ .[] | { timestamp: (if .blk? then (.blk.t | strftime(\"%Y-%m-%d %H:%M\")) else null end), signalId: .out[0].h5, totalChunks: .out[0].h4 } ]" }, // Payment/Exchange
                { "f": "[ .[] | { timestamp: (if .blk? then (.blk.t | strftime(\"%Y-%m-%d %H:%M\")) else null end), signalId: .out[0].h5, totalChunks: .out[0].h4, p2shSubScript: .out[0].h6} ]" } // Payment/Collaborate
            ]
        }
        this.qQuery = {
            "find": {
    ...
    async getSignalMetadata(msgType, txid=null, tokenOrOracleId=null, apiKey=null) {
        ...
        for(let i = 0; i< metadata.length; i++) {
            // exactUtxoIndex can be present in both message types
