    ...
    constructor(network) {
        ...
        this.jqQueries = {
            ...
            payment: [
                ...
                { "f": "[ .[] | { timestamp: (if .blk? then (.blk.t | strftime(\"%Y-%m-%d %H:%M\")) else null end), signalId: .out[0].h5, totalChunks: .out[0].h4, p2shSubScript: .out[0].h6, URI: \"swap:\\(.tx.h)\" } ]" } // Payment/Collaborate
            ]
        }
        this.qQuery = {
            "find": {
