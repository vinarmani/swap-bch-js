...
module.exports = class BfpBitdb {
    ...
    constructor(network) {
        ...
        this.jqQueries = {
            signal: [
                ...
                { "f": "[ .[] | { timestamp: (if .blk? then (.blk.t | strftime(\"%Y-%m-%d %H:%M\")) else null end), oracleBfp: \"bitcoinfile:\\(.out[0].h4)\", compilerId: .out[0].s5, compilerVersion: .out[0].s6, contractPartyIndex: .out[0].h7, pubKey: .out[0].h8, exactUtxoTxId: .out[0].h9, exactUtxoIndex: .out[0].h10, terms: .out[0].h11, appendedScriptPubKey: .out[0].h12, appendedSats: .out[0].h13, URI: \"swap:\\(.tx.h)\" } ]" }, // Signal/Escrow
    ...
    async getSignalMetadata(msgType, txid=null, tokenOrOracleId=null, address=null) {
        ...
        // If no txid, search by tokenId or OracleId
            ...
            if (address)
                qQuery.find["out.e.a"] = address.replace('bitcoincash:', '')
        }

        let metadata = await this.getFileMetadata(qQuery, jqQuery)
    ...
    async getPaymentMetadata(msgType, txid=null, signalId=null, address=null) {
        ...
        // If no txid, search by tokenId or OracleId
            ...
            if (signalId)
                qQuery.find["out.h5"] = signalId.replace('swap:', '')
            if (address)
                qQuery.find["out.e.a"] = address.replace('bitcoincash:', '')
        }
